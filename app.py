...
BASE_URL_NEW = "https://campaign.mylibby.ai/campaigns/{campaign_id}/contacts"
...
GYMS = {
    ...
    # --- Cancelled Member Month 1 (old API) ---
    ...
    # --- Cancelled Member Month 2 (old API) ---
    ...
    # --- Cancelled Member Month 3 (old API) ---
...
def forward_to_replify(gym_entry, data, gym_name):
    ...
    # Determine URL and API key based on new vs old
    if isinstance(gym_entry, dict) and gym_entry.get("new"):
        url = BASE_URL_NEW.format(campaign_id=gym_entry["campaign_id"])
        api_key = REPLIFY_API_KEY_NEW
    else:
        campaign_id = gym_entry if isinstance(gym_entry, str) else gym_entry["campaign_id"]
        url = BASE_URL_OLD.format(campaign_id=campaign_id)
